...
Widget::Widget(Ort::Env& env, const wchar_t* model_path, Ort::SessionOptions& session_options)
{
...
//修改图片大小
cv::Mat Widget::resizeImage(const cv::Mat& image, const cv::Size& size, bool letterbox_image) {
...
//归一化
cv::Mat Widget::preprocess_input(cv::Mat& image) {
	image.convertTo(image, CV_32F, 1.0 / 255);
	return image;
}


std::vector<float> Widget::matToVector(const cv::Mat& img) {
...
std::vector<float> Widget::outputs(const std::string& img_path, const std::vector<int64_t>& inputTensorShape){
